...
# You have set of 20 points in which some are noisy. Find set of 10 points that are closest to each other.
# via union find make union of points based upon some threshold distance
# at the end check if there are at least 10 points in any one group
# if not then return false
# else return mean of those 10 points
...
# 		# print("peri= ", peri, " area= ", area)

# 		# print(len(approx))
# 		# if len(approx) <= 4 or len(approx) > 10: continue
# 		if (len(approx) == 3 or len(approx) == 4) and area > 1000.0:
# 		# if (len(approx) == 3 or len(approx) == 4):
# 		# if (len(approx) == 3):
...
# 		cv2.circle(image, (res.x, res.y), 3, (0,255,0), -1)
# 		cv2.line(image, (wd/2, ht/2), (res.x, res.y), (136,0,108), 2)
# 		settings.err_x = res.x-(wd/2)
# 		settings.err_y = res.y-(ht/2)
# 		print(wd/2, ht/2, res.x, res.y, (res.x-(wd/2)), (res.y-(ht/2)))
# 		talker()
# 		# counter = 0
# 	else:
